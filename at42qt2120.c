...
void cap_sense_io_init(void)
{
	...
	cyhal_gpio_init(PIN_CT_IRQ, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_NONE, 0);
}
...
void AT42QT2120_write_reg(uint8_t reg, uint8_t value)
{
	...
	cyhal_i2c_master_write(&i2c_master_obj, AT42QT2120_SUBORDINATE_ADDR, write_buffer, AT42QT2120_WRITE_PACKET_SIZE, 0, true);
...
uint8_t AT42QT2120_read_reg(uint8_t reg)
{
	...
	/* a restart condition. */
	write_buffer[0] = reg;
	cyhal_i2c_master_write(&i2c_master_obj, AT42QT2120_SUBORDINATE_ADDR, write_buffer, 1, 0, false);
	...
	// of data.
	cyhal_i2c_master_read(&i2c_master_obj, AT42QT2120_SUBORDINATE_ADDR, read_buffer, 1, 0, true);
